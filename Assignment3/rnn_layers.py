...
def rnn_step_backward(dnext_h, cache):
	...
	##########################################################################
	prev_x, prev_h, prev_Wx, prev_Wh, prev_b = cache
	dx = np.matmul(dnext_h * (1 - np.tanh(np.matmul(prev_x, prev_Wx) + np.matmul(prev_h, prev_Wh) + prev_b)**2), prev_Wx.T)
	dprev_h = np.matmul(dnext_h * (1 - np.tanh(np.matmul(prev_x, prev_Wx) + np.matmul(prev_h, prev_Wh) + prev_b)**2), prev_Wh.T)
	dWx = np.matmul(prev_x.T, dnext_h * (1 - np.tanh(np.matmul(prev_x, prev_Wx) + np.matmul(prev_h, prev_Wh) + prev_b)**2))
	dWh = np.matmul(prev_h.T, dnext_h * (1 - np.tanh(np.matmul(prev_x, prev_Wx) + np.matmul(prev_h, prev_Wh) + prev_b)**2))
	db = np.sum(dnext_h * (1 - np.tanh(np.matmul(prev_x, prev_Wx) + np.matmul(prev_h, prev_Wh) + prev_b)**2), axis=0)
	##########################################################################
	#                               END OF YOUR CODE                             #
	##########################################################################
...
def rnn_forward(x, h0, Wx, Wh, b):
	...
	for t in range(T):
		if t == 0:
			h_output, cache_output = rnn_step_forward(x[:, t, :], h0, Wx, Wh, b)
		else:
			h_output, cache_output = rnn_step_forward(x[:, t, :], h[t - 1], Wx, Wh, b)
		h.append(h_output)
...
def rnn_backward(dh, cache):
	...
	for t in range(T):
		if t != 0:
			dx_temp, dprev_h_temp, dWx_temp, dWh_temp, db_temp = rnn_step_backward(dh[:, T - t - 1, :], cache[T - t - 1])
		else:
			dx_temp, dh0, dWx_temp, dWh_temp, db_temp = rnn_step_backward(dh[:, T - t - 1, :], cache[T - t - 1])
		dx[T - t - 1] = dx_temp
		dWx[T - t - 1] = dWx_temp
		dWh[T - t - 1] = dWh_temp
		db[T - t - 1] = db_temp
	dx = np.stack(dx, axis=1)
	dWx = np.sum(np.stack(dWx, axis=0), axis=0)
	dWh = np.sum(np.stack(dWh, axis=0), axis=0)
	db = np.sum(np.stack(db, axis=0), axis=0)
	##########################################################################
	#                               END OF YOUR CODE                             #
	##########################################################################
